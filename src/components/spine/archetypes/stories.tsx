...
export function pickHoodCompareInstances(): Instance[] {
  return hoodHandles().filter((h) => hoodServes(h, "compare")).map((h) => ({ h, c: buildHoodCompare(HOOD_INSTANCES[h].city, HOOD_INSTANCES[h].focus) })).filter((x) => x.c).map(({ h, c }) => ({ iso2: hoodKey(h, "compare"), why: `hood block 03: ${c!.rows.length} districts, what a shop takes there for three trades, measured (cafes, hair and beauty, dental), the best cell of each ticked, ${c!.dashed} thin cell${c!.dashed === 1 ? "" : "s"} dashed${c!.home ? `, ${c!.home} the home row, tinted` : ", no home row"}, no flags` }));
}
