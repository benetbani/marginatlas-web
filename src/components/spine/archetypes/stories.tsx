...
const lastsWhy = (l: NonNullable<ReturnType<typeof buildLasts>>) => `trade block 09: still trading after five, one and three years (${l.values.yr5} / ${l.values.yr1} / ${l.values.yr3}) off the shard, year five at 30 over years one and three at 16 (the worked figure since 2026-09-24), no slope, no myth sentence`;
  ...
  return (
    <div data-stories="worked-figure">
      ...
      {cell.filter((c) => cellServes(c.key, "lasts")).map((c) => {
        ...
        if (!l) return null;
        return <Story kind="worked-figure" key={cellLastsKey(c)} iso2={cellLastsKey(c)} why={lastsWhy(l)}><div style={{ maxWidth: 520 }}><LastsCard id={`lasts-cell-${c.key}`} lasts={l} /></div></Story>;
      })}
    </div>
  );
}
...
const splitWhy = (s: NonNullable<ReturnType<typeof buildSplit>>) =>
  s.state === "withheld" ? `trade block 05 withheld: the ${s.feed === "shard" ? "shard's" : "sector's"} lines (${Math.round(s.linesPct)}) and the net (${s.netText}) come to more than a hundred; the net at 30, the stated line where the bar would stand, ${s.mix.length ? `the costs split among themselves (${s.mix.length} segments of every $100 spent), ` : ""}the plus at the foot`
...
export function MonthBarsStories({ cell = [] }: { cell?: CellHeroInstance[] }) {
  return (
    <div data-stories="month-bars">
      {cell.filter((c) => cellServes(c.key, "market")).map((c) => {
        const m = buildMarket(c.seed?.meta?.industry_id);
        if (!m || !m.months) return null;
        return <Story kind="month-bars" key={cellSwingKey(c)} iso2={cellSwingKey(c)} why={swingWhy(m)}><div style={{ maxWidth: 693 }}><SwingCell market={m} /></div></Story>;
      })}
    </div>
  );
}
