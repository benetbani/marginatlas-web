 ...
 *    under the figure it pictures (the country hero's tax on profit), where
 *    a label row would print that figure a second time. The two parts are
 *    named under the cells instead, the filled one at the left (`part`), the
 *    rest at the right (`rest`): labels, never a sentence.
 */
import * as React from "react";

export const SEGMENT_BAR_CELLS = 25;

export function SegmentBar({ label, value, total = 100, figure, unit, chip, rest, part, bare = false, segments = SEGMENT_BAR_CELLS }: { label: string; value: number; total?: number; figure: string; unit: string; chip?: React.ReactNode; /** The unfilled part named, one micro line under the segments at the right ("Visitors 38%"), for a share of a whole whose remainder has a name. */ rest?: string; /** The filled part named under the cells at the left, the bare form's. */ part?: string; /** No label row: the figure is printed right above the bar (THE BARE FORM). */ bare?: boolean; segments?: number }) {
