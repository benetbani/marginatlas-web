  ...
  return (
    <Box id={id} className="flex flex-col">
      ...
      <div className="[container-type:inline-size]">
        ...
        {/* A PART AND ITS ROW ARE ONE (goal 2026-09-26, M2): a chain's length on the bar and its line in the list light together. */}
        <Marks label={M.kicker} data-archetype="market-hold" data-visual="1" data-wedges={String(ordered.length)}>
          ...
          <div className="flex h-9 w-full gap-0.5 overflow-hidden rounded-lg" role="img" aria-label={ordered.map((p) => `${p.name} ${p.pct}%`).join(", ")}>
            {ordered.map((p) => (
              <span key={p.key} data-wedge={p.key} data-part-key={p.key} data-readout-figure={`${p.pct}%`} data-readout-words={p.name} className="block h-full min-w-0.5 first:rounded-l-lg last:rounded-r-lg" style={{ width: `${((p.pct / total) * 100).toFixed(2)}%`, background: fill(p) }} />
            ))}
          </div>
          ...
          <div className="mt-3 grid gap-x-6 [@container(min-width:560px)]:grid-cols-2">
            {[ordered.slice(0, Math.ceil(ordered.length / 2)), ordered.slice(Math.ceil(ordered.length / 2))].map((col, ci) => (
              <div key={ci}>
                {col.map((p) => (
                  <div key={p.key} data-row={p.key} data-part-key={p.key} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-3 border-t border-[var(--c-border)] py-2">
