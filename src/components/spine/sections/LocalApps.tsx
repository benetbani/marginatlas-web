          ...
          {data.jobs.map((j) => {
            ...
            <section key={j.key} data-job={j.key} className="mb-5 min-w-0 break-inside-avoid">
              ...
              <ol className="m-0 list-none divide-y divide-[var(--c-border)] p-0">
                {j.apps.map((a) => (
                  <li key={a.name} data-row={a.name} title={a.note || undefined} className={`grid max-w-none items-center gap-x-3 py-2 ${allFree ? "grid-cols-[auto_minmax(0,1fr)]" : "grid-cols-[auto_minmax(0,1fr)_auto]"}`}>
                    ...
                    {allFree ? null : <Fig className="whitespace-nowrap text-right text-[length:var(--t-body)] font-semibold leading-tight text-[var(--c-ink)]">{a.fee}</Fig>}
                  </li>
                ))}
              </ol>
            </section>
