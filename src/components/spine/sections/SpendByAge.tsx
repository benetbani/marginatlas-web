...
export function SpendByAge({ id = "spend-by-age", data }: { id?: string; data: SpendByAgeData }) {
  ...
  return (
    <Box id={id} className="flex flex-col">
      ...
      <div data-archetype="spend-by-age" data-visual="1" data-bars={String(bars.length)} data-wedges={String(data.bands.length)} className="grid flex-1 content-between gap-4">
        {bars.map((bar) => (
          <div key={bar.key} data-row={bar.key}>
            ...
            <div className="flex h-9 w-full gap-0.5 overflow-hidden rounded-lg" role="img" aria-label={`${bar.name}: ${data.bands.map((b) => `${b.label} ${Math.round(bar.value(b))}%`).join(", ")}`}>
              {data.bands.map((b) => {
                ...
                return (
                  <span key={b.key} data-wedge={b.key} className="relative flex h-full min-w-0.5 items-center justify-center overflow-hidden first:rounded-l-lg last:rounded-r-lg" style={{ width: `${v}%` }}>
