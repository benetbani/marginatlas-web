...
export function Pie({ share, aria, className = "h-24 w-24" }: { share: number; aria: string; className?: string }) {
  ...
  return (
    <svg data-chart="pie" data-visual="1" data-share={String(Math.round(share * 100))} viewBox="0 0 100 100" role="img" aria-label={aria} className={`block shrink-0 ${className}`}>
      ...
      <path d={`M50,50 L50,0 A50,50 0 ${large} 1 ${x.toFixed(3)},${y.toFixed(3)} Z`} fill="var(--terra)" stroke="var(--c-card)" strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  );
}
