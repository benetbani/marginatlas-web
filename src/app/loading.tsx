...
export default function Loading() {
  return (
    <div className="py-10 animate-pulse">
      ...
      <div className="h-10 w-3/4 max-w-2xl bg-cream-200 rounded mb-3" />
      <div className="h-4 w-2/3 max-w-xl bg-cream-100 rounded mb-2" />
      <div className="h-4 w-1/2 max-w-md bg-cream-100 rounded" />

      <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-24 rounded-2xl bg-cream-100 border border-parchment"
          />
        ))}
      </div>
