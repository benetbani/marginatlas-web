...
for (const w of [1280,375]){
  ...
  const h=await p.evaluateHandle(()=>{
    const n=[...document.querySelectorAll('*')].find(e=>e.children.length===0&&/Where each \$100 of sales goes/.test(e.textContent||''));
    let c=n; for(let i=0;i<8&&c;i++){c=c.parentElement; if(c&&/rounded-\[14px\]/.test((c.className||'').toString()))return c;}
    return n;});
  const el=h.asElement();
  if(el) await el.screenshot({path:'scratchpad/shots-close/SPLIT-'+w+'.png'});
