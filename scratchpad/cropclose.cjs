...
for (const w of [1280,375]){
  ...
  const h=await p.evaluateHandle(()=>{
    const n=[...document.querySelectorAll('*')].find(e=>e.children.length===0&&e.textContent.trim()==='The pick, and where to take it');
  ...
  if(el) await el.screenshot({path:'scratchpad/shots-close/CLOSE-'+w+'.png'});
