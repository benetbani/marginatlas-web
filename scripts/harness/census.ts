 ...
 * and a builder called outside the enclosing component is not seen. The
 * ledger is the declaration: whether a render carries the accents it declares
 * is measured by the loud-seats gate, never here.
 */
...
function render(sections: Section[], files: string[], loud: Map<string, LoudDeclaration>): string {
...
const block = render(sections, files, loud.declarations);
