 ...
 * A finding that appears at more than one width is reported ONCE per file,
 * with every width it was seen at named in its row, not three times over:
 * this file's own fixture has no responsive rule at all, so without that
 * fold every content rule would print three times and BLOCK FLOOR's twelve
 * would become something closer to thirty, which is the exact averaging-
 * style distortion check_readability.mjs's own review wave was built to
 * stop repeating.
 *
...
function inPage(ctx) {
  ...
  /* BLOCK FLOOR: "count the page's blocks against its floor, 21 and 17." */
  if (wide && floor != null) {
  ...
  /* ROW SENTENCE: "a label over three words, or a spectra pole over three
     words or 24 characters, is a copy fault." */
  for (const label of document.querySelectorAll("[data-label]")) {
    if (!label.getClientRects().length || hiddenFromSight(label)) continue;
...
for (const file of files) {
  ...
  /* FOLDED ACROSS WIDTHS, not printed three times over: see the header
     comment on why the same (card, rule) seen at more than one width is one
     finding, named with every width it held at, not three near-duplicates. */
  ...
  for (const w of WIDTHS) {
    ...
    for (const f of found) {
      const key = `${f.id}␟${f.rule}`;
      const prior = perFile.get(key);
      if (prior) prior.widths.add(w);
      else perFile.set(key, { id: f.id, rule: f.rule, detail: f.detail, widths: new Set([w]) });
    }
  ...
  for (const v of perFile.values()) {
    reds.push({ name, w: [...v.widths].sort((a, b) => b - a).join("/"), id: v.id, rule: v.rule, detail: v.detail });
  }
