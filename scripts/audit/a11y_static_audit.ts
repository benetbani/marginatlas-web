...
function main() {
  ...
  md.push("");
  /* NO TIMESTAMP. It carried `Generated ${new Date().toISOString()}` until the
     audit joined the prebuild chain at tick 17, at which point every run of the
     chain rewrote a TRACKED file and left the tree dirty. The loop halts on an
     unexpected dirty tree, so that one line would have stopped every future tick
     before it started.

     Determinism is worth more than the stamp anyway: with the output stable, git
     itself becomes the change detector. A diff on this file now means a FINDING
     changed, which is the only thing worth noticing, and the question the
     timestamp answered is answered better by the commit date. */
  md.push(`Regenerated by \`npx tsx scripts/audit/a11y_static_audit.ts\`.`);
  md.push("");
