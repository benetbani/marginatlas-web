/**
 * proof_bench , the before and after sheet for "kept per $100, by trade".
 *
...
const OUT = "docs/loop/artifacts/bench/bench-before-after.html";
const before = readFileSync("scratchpad/bench-before.html", "utf8");
const after = readFileSync("scratchpad/bench-after.html", "utf8");
const tw = readFileSync("scratchpad/tw-bench.css", "utf8");
...
<meta charset="utf-8">
<title>Kept per $100 by trade, before and after</title>
...
<h1>Kept per $100 by trade, before and after</h1>
<p>Both halves are the real section, server rendered. <b>Every word and figure is
byte for byte identical</b>, checked node by node.</p>
<p><b>Narrow this window to phone width.</b> In the lower one the scale is squeezed
into about fifty pixels between a name column nailed at 110 and a figure column
nailed at 40, so the dots barely have room to differ. In the upper one the scale
takes its own line under the name and its figure, and the spread between trades
reads properly.</p>
<p>Two more went with it. <b>Trade names were cut off and the rest of the word put
in a tooltip</b>, which is not a carrier at all on a touch screen: there is no
hover, so the end of the name simply did not exist for the reader most likely to
be holding a phone. Names wrap now and the tooltips are gone. And the dots were a
WARM grey on a palette whose rule is terracotta plus strictly cool neutrals;
they are the cool neutral of the same weight now, and a token rather than a
literal.</p>
