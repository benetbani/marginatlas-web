...
const DECISIONS = [
  ...
  ["The terracotta detail inside every section icon",
   "Every one of the 36 section icons across the four pages carries a small terracotta detail: a tick, a dot, a stroke, two to five pixels across inside a fourteen-pixel glyph. It is uniform, so it reads as a deliberate choice in the icon family rather than an accident, and I found it only because a check started seeing colour drawn inside pictures.",
   "Two of your own rules pull against each other here and I will not pick for you. One says the accent marks the answer and nothing else; the other says no colour on chrome, and it names icon tiles and tick marks specifically. Thirty-six quiet ticks do not shout, but they do mean the accent appears in every section whether or not that section has an answer to point at. Keep it as house identity, or drain the icons to ink and let terracotta mean one thing. I have changed nothing."],
