...
| G33 | Zero duplicate surfaces: one way to solve each problem | `04-CONSOLIDATION.md`. Converge and redirect, never delete | OPEN — 6 percentile charts disagreeing on axis (2 logarithmic), 5 "where each $100 goes", month-of-year charts: **RE-MEASURED 2026-08-21 and this claim was wrong.** Four render for a reader and every one is already on a zero baseline or is a deviation chart, which centres on zero by definition. The one non-zero floor found is a pay-by-role RANGE chart, not a month chart, where a shared scale is defensible because the reader is reading the span rather than the magnitude, ~~9 gauge geometries~~ and ~~3 components named `Waterfall`~~ , **both re-measured 2026-08-21 and both wrong.** Counting only forms where an arc length or a needle angle encodes a value, the site has **one** semicircular gauge, **one** multi-segment donut and **two** score rings. The nine came from counting `Math.PI` and `strokeDasharray`, which also match radial LAYOUT and decorative dashes. `Waterfall` is **2**, not 3 |
...
| G35 | One table primitive with `scope`, tabular figures, right-aligned numerics and a sticky header | count tables missing each property | OPEN, AND RE-MEASURED 2026-08-21 because three of the four figures here were wrong. **19** files hold a `<table>`, **11** of them with no `scope` at all (not 13). **One** has a sticky header, not zero. And the tabular-figures claim was simply false: `tabular-nums` appears **405** times across the site, including on line 48 of `Money.tsx`, the very file this row named as lacking it. The real remaining defects are `scope` on 11 files and sticky headers on 18 of 19 |
