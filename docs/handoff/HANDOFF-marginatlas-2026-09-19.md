...
Short on purpose. The checkup of the same day (`E:/atlas/design/loop/build/CHECKUP-2026-09-19.md`) measured the reading tax at about 100,000 words before a first edit; this page is the whole mandatory read for the next session, and it points at the two files that are the state of record.

## Where everything is

- **Website** `E:/atlas/website`, branch `main`, HEAD `44877f5e`, pushed; production (`marginatlas.com`) serves it since 11:47 UTC (three deploys today: the first failed on three harness gates that stopped for want of a browser on a build server, fixed; the third fixed the how-to page's 404 status). The chain holds 155 gates and runs inside every Vercel build (`vercel.json` pins `npm run build`; the `prebuild` hook is the chain).
