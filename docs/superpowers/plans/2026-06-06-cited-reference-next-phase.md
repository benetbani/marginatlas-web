...
**2026-06-07 — the cost-to-open page SHIPPED to production (commits `f84b4f0a` + dedup `bf0439ee`).** New route `src/app/[country]/[geo]/[industry]/opening/page.tsx` (bounded ~20-URL flagship prerender + `dynamicParams` ISR, `notFound()` on untrusted), built in the clean-data-tool register: `OpeningHero` (total-to-open + the break-in score + a one-line verdict, image-free), `OpeningChecklist` (capital / time / permits / first hires, modeled-hinted), `OpeningPayback` (breaks-even line), `OpeningComparisons` (cheaper/dearer to open elsewhere + easier/harder businesses here, band-badged). Cross-linked FROM the cell via a new additive `footer` slot on `DataSection` ("See the full opening guide"). A verdict/headline duplication was caught in the screenshot and fixed (`BreakInMasthead` gained `showHeadline`, the hero passes false). Verified on preview desktop + mobile; gates + build green; live.

**Phase 1 is essentially DONE.** Remaining tidy-ups (small, optional): cross-link to the opening page from the homepage break-in beat rows, the comparison page, and the Extremes rows (only the cell cross-link was added); and a sitemap entry for the new route. Then move to **Phase 2 (weave the break-in score through comparison / activity / country / city)**, with the **hospitality-sqm refinement** (Phase 3) available to pull forward to lift small inns off the take-home floor.

**2026-06-07 — Phase 2 high-value surfaces SHIPPED to production.**
- **Comparison pages** (commit `2be96844`): a per-place break-in column + an "Easiest to break in" read + cost-to-open links on `/industries/[industry]/across`. The per-column score was consolidated into `across_cities.ts` (`breakInForColumn`) and the Extremes board now READS that stored value, so there is one literal source of truth (no drift). Live.
- **Country + place pages** (commit `a7cb0da5`): an "Easiest businesses to break into here" panel (`src/components/countries/EasiestToBreakIn.tsx` + `buildEasiestToBreakIn` in `country_board.ts`) on `/[country]` and `/[country]/[geo]`, scores verified masthead-matched (0 mismatches). Renders on countries and US states; **world-city pages self-omit** (honest) because their per-industry cells are too thin/untrusted to score 3+ defensibly. Live.

**The break-in rating now follows the buyer across every data-bearing surface:** cell masthead, homepage beat, Extremes rankings, the comparison column, country/state panels, and the cost-to-open page. The "product feels thin" risk is thoroughly answered.

**Phase 2 remainder (lower value / data-gated):**
- **Activity pages** (one business across places) still lack the score; it is similar to the comparison page, so lower marginal value.
- **World-city panels** are the real gap, and the lever is NOT more wiring but **Phase 3 big-economy + city depth** (more trusted per-industry city cells). Pull Phase 3 forward to light up the city panels.
- Small cross-link tidy-ups (homepage beat / comparison / extremes to the opening page) + a sitemap entry remain.

**2026-06-07 — Phase 3 trust finish, item 1: the currency/scale overstatement FIXED (commits `dc5dcd43` + narrative `24bc62f2`).** A SECOND live #1-bar breach found and fixed. Non-US cells in small / low-income economies displayed wrong-scale money (CA software read $6.3M revenue / $1.4M owner take-home, 12x the US; QA software $5.2M; CA restaurants $1.95M / $221K), from a wrong-scale "verified median" anchor (`industry_medians_v1.json`, its own US entry 15x off) that the pipeline trusted as a preferred override, plus the same defect on the extrapolated path.
- **Fix:** a wealth-normalized relative-outlier DASH at the shared suppression chokepoint (`plausibility_suppression.ts` + `enforceSanity`): `revenue_per_firm / (globalMedian x country wealth multiplier) > 2.5` dashes the whole revenue waterfall + derived take-home + break-in score, via the existing `_revenueSuppressed` cascade. Founder chose the **aggressive 2.5x** threshold ("money numbers never wrong" beats completeness).
- **Prevalence:** 18.8% of cells dash, entirely concentrated in wrong-scale small/low-income economies; **zero** dashes in any major rich economy (US/GB/DE/FR/JP/IT/ES/NL/SE). Verified: the 3 named suspects dash, every rich-country control byte-identical.
- **Narrative leak caught in screenshot + fixed:** the pre-generated cell narratives (`data/content/cell_narratives_v1.json`, frozen prose) still cited the suppressed revenue ("brings in roughly $21.6M"). Gated the narrative render on `cell.revenue_per_firm != null` so a dashed cell drops its revenue-citing prose; the US control keeps its narrative. (Lesson: a data suppression must cascade into frozen editorial text, not just the live stat tiles.)

**Two live #1-bar breaches fixed this session** (negative take-home, currency overstatement), both found by going deeper, both fixed with the dry-run/show/verify discipline.

**Phase 3 trust finish, remaining items:** wrong-industry remaps (silent mislabels + the reverse-map first-write-wins bug); extrapolated_cells band dedupe (DB migration, founder applies); the hospitality-sqm refinement (lift small inns off the take-home floor). Then big-economy + city depth (the lever for the world-city break-in panels).
